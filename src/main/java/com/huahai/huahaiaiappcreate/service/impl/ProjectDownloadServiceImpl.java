...
import com.huahai.huahaiaiappcreate.service.ProjectDownloadService;
...
public class ProjectDownloadServiceImpl implements ProjectDownloadService {
    ...
    public void downloadProjectAsZip(String projectPath, String downloadFileName, HttpServletResponse response) {
        ...
        // 3. 创建文件过滤器，过滤不允许的文件和目录
        FileFilter fileFilter = file -> {
            return isPathAllowed(projectDir.toPath(), file.toPath());
        };
        try {
            // 使用 hutool 的 zipUtil 方法进行打包，使压缩后的目录直接输出到响应流中
            ...
            log.error("项目打包失败：{}", downloadFileName, e);
